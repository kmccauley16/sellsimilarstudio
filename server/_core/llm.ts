...
export type InvokeParams = {
  ...
  maxTokens?: number;
  max_tokens?: number;
  /** GPT-family completion cap; keeps reasoning tokens from exhausting visible output. */
  ...
  /** Optional retry cap for a caller that must make exactly one upstream attempt. */
  maxRetries?: number;
};
...
export async function invokeLLM(params: InvokeParams): Promise<InvokeResult> {
  ...
  const retryLimit = typeof maxRetries === "number" && Number.isInteger(maxRetries)
    ? Math.max(0, Math.min(maxRetries, RETRY_MAX_RETRIES))
    : RETRY_MAX_RETRIES;
